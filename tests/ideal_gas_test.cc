...
TEST_CASE("Wall collisions", "[wall][collision]") {
  ...
  SECTION("Collision with right wall, center of mass past wall") {
    // Particle not touching wall juuuuuust yet but moving really fast
    container.Generate_particle(39, 25, 15, 0, 10.0f, "red");
    container.Update();
    REQUIRE(container.Get_particles().at(0)->GetPosition().x == 54); // center of mass is past wall
    REQUIRE(container.Get_particles().at(0)->GetPosition().y == 25);
    REQUIRE(container.Get_particles().at(0)->GetVelocity().y == 0);
    REQUIRE(container.Get_particles().at(0)->GetVelocity().x < 0);
    container.Update();
    REQUIRE(container.Get_particles().at(0)->GetPosition().y == 25);
    REQUIRE(container.Get_particles().at(0)->GetPosition().x == 39);
  }
